...
export function DocumentationSection({ meta, schema }: DocumentationSectionProps) {
  if (!meta && (!schema || !hasSchemaDescriptions(schema))) {
    return null;
  }
  
  // Extract all property descriptions including nested ones
  const propertyDescriptions = schema ? extractPropertyDescriptions(schema) : [];
              ...
              >
                ...
                >
                  ...
                  >
                    {prop.path}
                    {prop.required && (
                      <Typography component="span" color="error.main" sx={{ ml: 0.5, fontWeight: 700 }}>
                        *
                      </Typography>
                    )}
                  </Typography>
...
function extractPropertyDescriptions(
  ...
  for (const [propName, propSchema] of Object.entries(schema.properties)) {
    ...
    const propIsRequired = parentIsRequired && (schema.required || []).includes(propName);
...
function hasSchemaDescriptions(schema: JSONSchema7Object): boolean {
  ...
  if (schema.properties) {
    const hasDirectDescriptions = Object.values(schema.properties).some((prop) => {
